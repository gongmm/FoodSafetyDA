...
class Crawler_4(CrawlSpider):
    name = 'crawler_4'  #scrapy crawler ***指定名字

    #读取csv文件，将url保存为list
    def read_csv(self):
        filename = '/Users/Amanda/Desktop/大四/毕设/爬虫/url分类爬虫/urls/newspeople_4.csv' #需要读取对的文件路径
    ...
    def parse(self, response):
        ...
        div_bf = bf.find('div', attrs={'class': 'show_text'})  # content
        p = div_bf.find_all('p')
        content =''
        for each in p:
