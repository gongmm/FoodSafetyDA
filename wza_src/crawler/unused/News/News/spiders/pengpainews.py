...
class PengPaiNews(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        start_url = 'https://www.thepaper.cn/load_search.jsp?k='+keyword+'&pagesize=10&searchPre=all_0:&orderType=1&pageidx='+str(self.page)
    ...
    def parse(self, response):
        if response.body!='':
            ...
            for search in searches:
                ...
                title=search.xpath('./h2/a/text()').extract()
                pubdate=search.xpath('./div[@class="search_trbs"]/a/preceding-sibling::span[1]/text()').extract_first()
                print (pubdate)
                yield Request(url=self.url_prefix+link,callback=self.parsedetail,meta={"source":self.url_prefix+link,"title":title,"pubdate":pubdate})
            self.page=self.page+1
            next_page='https://www.thepaper.cn/load_search.jsp?k='+self.keyword+'&pagesize=10&searchPre=all_0:&orderType=1&pageidx='+str(self.page)
