   ...
   # ]
    ...
    def parse(self, response):
        ...
        for vrwrap in pic_content:
            ...
            source=vrwrap.xpath("./div[@class='strBox']/div[@class='str_info_div']/div[@class='fb']/cite/text()").extract_first()
            source_url=""
            if not source is None:
                source_url = source.split('-')[0].strip()
            if not url is None:
                yield scrapy.Request(url="https://www.sogou.com"+url,callback=self.parsedetail,meta={'source':source_url})
