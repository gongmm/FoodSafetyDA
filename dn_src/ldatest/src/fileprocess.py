...
class fileprocess:
    ...
    def shipin_test(self):
        with open('../csv/testcsv/shipinprocess.csv','r',encoding='utf-8') as csvfile:
            reader = csv.DictReader(csvfile)
            content_list = [row['content'] for row in reader]
            i=0
            for content in content_list:
    ...
    def topicAnalyze(self,corpus):
        ...
        train_index = sorted(random.sample(range(len(corpus)), train_size))###随机选取下标
        ...
        grid = dict()
        t0 = time()
        for i in range(1,300,5): ###300个主题，以5为间隔
            grid[i] = list()
            n_topics = i
