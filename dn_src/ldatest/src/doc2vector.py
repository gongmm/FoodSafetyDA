...
class Doc2Vector():
    ...
    def gettopicdoc(self):
        for topicid in range(self.topicnum):
            ...
            for i in range(len(doctuples)):
                with open('../txt/shipintest/nlp_test' + doctuples[i].get('docid') + '.txt', 'r',
                          encoding='utf-8') as f:
                    ...
                    with open(
                            '../txt/topic_doc_jieba/topic' + doctuples[i].get('topicid') + '/topic' + doctuples[i].get(
                                'topicid') + '_doc' + doctuples[i].get('docid') + '.txt', 'w',
                        ...
                        for word in wordList:
                            for i in range(len(word)):
                                if word[i].encode('utf-8') == '\n' or word[i].encode('utf-8') == 'nbsp' or word[
                                    i].encode(
                                    'utf-8') == '\r\n':
                                    continue
                                line += word[i]
        ...
        for topicid in range(self.topicnum):
            ...
            for i in range(len(doctuples)):
                ...
                with open('../txt/shipintest/nlp_test' + doctuples[i].get('docid') + '.txt', 'r',
                    ...
                    with open('../txt/topic_doc_pk/topic' + doctuples[i].get('topicid') + '/topic' + doctuples[i].get(
                            'topicid') + '_doc' + doctuples[i].get('docid') + '.txt', 'w', encoding='utf-8') as fw:
                        doc = f.read()
                        reg_html = re.compile(r'<[^>]+>', re.S)
                        doc = reg_html.sub('', doc)  # 去掉html标签
                        doc = re.sub('[０-９]', '', doc)  # 去掉数字
                        doc = re.sub('\s', '', doc)  # 去掉制表符
                        # 使用模型对文档进行分词
                        rowList = [eachWord for eachWord in seg.cut(doc)]
                        wordList = []
                        removeStopwordList = []
                        '''停用词处理'''
                        stopWords = open('../corpus/stopwords-utf8.txt', 'r', encoding='utf-8').read()
    ...
    def train(self):
        ...
        for n in range(self.topicnum):
            # 先把所有文档的路径存进一个 array中，docLabels：topic文件夹下所有的txt文件名
