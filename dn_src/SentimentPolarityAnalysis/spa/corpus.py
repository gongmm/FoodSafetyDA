...
class Corpus:
    def __init__(self, filepath):
        ...
        re_split = re.compile("\s+")#匹配任何空格字符

        self.pos_doc_list = []
        self.neg_doc_list = []
        with open(filepath, encoding="utf-8") as f:
            for line in f:
                splits = re_split.split(line.strip())#以空格切割字符串
                if splits[0] == "pos":
                    self.pos_doc_list.append(splits[1:])#将正文档加入集合中
                elif splits[0] == "neg":
                    self.neg_doc_list.append(splits[1:])#将负文档加入集合中
                else:
                    raise ValueError("Corpus Error")

        self.doc_length = len(self.pos_doc_list)
        assert len(self.neg_doc_list) == self.doc_length #正集合和负集合的长度一样
